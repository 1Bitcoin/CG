...
namespace KG_LABA5
{
    class FillPoligons
    {
       ...
       {
            int xl = bordermaxmix[index].XL; // левая граница
            int xr = bordermaxmix[index].XR; // правая граница

            bool flag = false;

            for (int i = xl; i <= xr; i++)
           {
               if (bitmap.GetPixel(i, index).ToArgb() == drawcolor.ToArgb()) // если пиксел имеет граниченое значение
               {                     
                   flag = !flag;                   
               }

               if (flag == true) // присвоить цвет многоугольника
               {
